...
ND = NormalDist()
_BB = ["e5-base", "bge-base", "mxbai-large", "gte-base"]
_DSET = ["scifact", "nfcorpus", "arguana", "fiqa"]
MAIN = [(b, d) for b in _BB for d in _DSET]   # 4x4 main matrix
ROB = []                                       # FiQA promoted into the matrix
NAME = {"e5-base":"E5-base","bge-base":"BGE-base","mxbai-large":"MXBAI-large","gte-base":"GTE-base"}
...
def t_scalar_int8(R):
    ...
    return env("\n".join(rows),
        r"Scalar int8 vs.\ identity over per-query \ndcg{} across the 4\,$\times$\,4 matrix (16 backbone--dataset pairs). "
...
def t_main_pareto(R):
    ...
    return env("\n".join(rows),
        r"Main 4\,$\times$\,4 BEIR matrix: hypervolume (HV) and Pareto-set size from each "
...
def t_robustness(R):
    if not ROB:
        return "% (robustness folded into the 4x4 matrix; no separate robustness table)"
    rows=[]
    for bb,ds in ROB:
        d=load(bb,ds,R)
...
def main():
    ...
    ap.add_argument("--emit-dir", default=None, help="write one <label>.tex per table")
    a=ap.parse_args()
    R=a.results
    blocks=[
    ...
    if a.emit_dir:
        ...
        for _t,_b in blocks:
            m=_re.search(r"\\label\{tab:([A-Za-z0-9_-]+)\}", _b or "")
            if not m: continue
            open(os.path.join(a.emit_dir, m.group(1)+".tex"),"w").write(_b+"\n")
        print(f"% wrote per-table files to {a.emit_dir}/")
    if a.emit_tex:
