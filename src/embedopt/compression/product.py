...
def _kmeans(
    ...
    for _ in range(n_iter):
        ...
        for start in range(0, n, _DISTANCE_BATCH_SIZE):
            batch = data[start : start + _DISTANCE_BATCH_SIZE]
            d2 = ((batch[:, None, :] - centroids[None, :, :]) ** 2).sum(axis=2)
            assign = d2.argmin(axis=1)
            for j in range(k):
                mask = assign == j
                if mask.any():
                    sums[j] += batch[mask].sum(axis=0)
...
class ProductQuantizeCompressor:
    ...
    def transform(self, vectors: FloatArray) -> CompressedSet:
        ...
        for m in range(self.n_subspaces):
            sub = vectors[:, m * self._sub_dim : (m + 1) * self._sub_dim]
            for start in range(0, n, _DISTANCE_BATCH_SIZE):
                batch = sub[start : start + _DISTANCE_BATCH_SIZE]
                d2 = ((batch[:, None, :] - self._codebooks[m][None, :, :]) ** 2).sum(axis=2)
                codes[start : start + batch.shape[0], m] = d2.argmin(axis=1).astype(np.uint8)
